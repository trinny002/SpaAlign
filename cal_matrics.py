...
from metric import jaccard, Dice, F_measure, cluster_acc
...
def main(arg):
    ...
    # SpaMICS metrics
    SpaMICS_acc, SpaMICS_f1 = cluster_acc(np.array(GT_list), np.array(Our_list))
    print(f"SpaMICS     acc: {SpaMICS_acc:.6f}")
    print(f"SpaMICS     f1: {SpaMICS_f1:.6f}")
    ...
    with open(arg.save_path, 'w') as f:
        ...
        f.write(f"SpaMICS acc: {SpaMICS_acc:.6f}\n")
        f.write(f"SpaMICS f1: {SpaMICS_f1:.6f}\n")
